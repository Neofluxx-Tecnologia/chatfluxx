(function () {
  ...
  const CONFIG = {
    webhookUrl: 'https://webhooks.neofluxx.com/webhook/disparo-whatsapp-v2',
    get tenantKey() {
  ...
  window.nfxdLoadTemplates = async function() {
    ...
    if (!cfg.webhookUrl) {
      container.innerHTML = '<div class="nfxd-ld">Webhook não configurado.</div>';
      return;
    }
  ...
  window.nfxdLoadImages = async function() {
    ...
    if (!cfg.webhookUrl) {
      container.innerHTML = '<div style="font-size:11px;color:var(--amb);padding:8px 0">Webhook não configurado.</div>';
      return;
    }
  ...
  window.nfxdSubmit = async function() {
    const cfg = getConfig();
    if (!cfg.webhookUrl) {
      alert('Webhook não configurado.'); return;
    }
    const nome = (document.getElementById('nfxd-nome')?.value || '').trim();
    const data = document.getElementById('nfxd-data')?.value || '';
    const hora = document.getElementById('nfxd-hora')?.value || '';
  ...
  window.loadCampanhas = async function() {
    ...
    if (!cfg.webhookUrl) {
      container.innerHTML = '<div class="nfxd-ld">Webhook não configurado.</div>'; return;
    }
